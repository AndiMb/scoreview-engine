 ...
 */
// shadow copy of src/framework/draw/internal/fontsengine.cpp — one marked
// diff at the end: glyphRuns(), the render() loop with the SDF generation
// replaced by glyph identity. Compiled with the same forced prelude header
// as the submodule original (fontsengine_prelude.h).
...
// shadow diff: glyph-run access for the SVG writer. This is render()'s loop
// with the SDF generation replaced by glyph identity; face selection,
// substitution and advance arithmetic stay byte-for-byte the same so the SVG
// output uses exactly the metrics the layout measured with.
std::vector<FontsEngine::GlyphRun> FontsEngine::glyphRuns(const Font& f, const std::u32string& text) const
{
